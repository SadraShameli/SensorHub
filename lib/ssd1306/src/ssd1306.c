...
static esp_err_t ssd1306_write_data(
    ssd1306_handle_t dev, const uint8_t *const data, const uint16_t data_len
) {
...
static esp_err_t ssd1306_write_cmd(
    ssd1306_handle_t dev, const uint8_t *const data, const uint16_t data_len
) {
    ssd1306_dev_t *device = (ssd1306_dev_t *)dev;
    esp_err_t ret;
...
static inline esp_err_t ssd1306_write_cmd_byte(
    ssd1306_handle_t dev, const uint8_t cmd
) {
...
void ssd1306_draw_num(
    ...
    for (i = 0; i < chLen; i++) {
        ...
        if (chShow == 0 && i < (chLen - 1)) {
            if (chTemp == 0) {
                ssd1306_draw_char(
                    dev, chXpos + (chSize / 2) * i, chYpos, ' ', chSize, 1
                );
                continue;
            } else {
                chShow = 1;
        ...
        ssd1306_draw_char(
            dev, chXpos + (chSize / 2) * i, chYpos, chTemp + '0', chSize, 1
        );
    }
}
...
esp_err_t ssd1306_init(ssd1306_handle_t dev) {
    ...
    ssd1306_write_cmd_byte(dev, 0xAE);
    ssd1306_write_cmd_byte(dev, 0x40);
    ssd1306_write_cmd_byte(dev, 0x81);
    ssd1306_write_cmd_byte(dev, 0xCF);
    ssd1306_write_cmd_byte(dev, 0xA1);
    ssd1306_write_cmd_byte(dev, 0xC0);
    ssd1306_write_cmd_byte(dev, 0xA6);
    ssd1306_write_cmd_byte(dev, 0xA8);
    ssd1306_write_cmd_byte(dev, 0x3f);
    ...
    ssd1306_write_cmd_byte(dev, 0xD9);
    ssd1306_write_cmd_byte(
        dev, 0xF1
    );
    ssd1306_write_cmd_byte(dev, 0xDA);
    ssd1306_write_cmd_byte(dev, 0xDB);
    ssd1306_write_cmd_byte(dev, 0x40);
    ssd1306_write_cmd_byte(dev, 0x8D);
    ssd1306_write_cmd_byte(dev, 0x14);
    ssd1306_write_cmd_byte(dev, 0xA4);
    ssd1306_write_cmd_byte(dev, 0xA6);

    const uint8_t cmd[2] = {0x20, 1};
    ssd1306_write_cmd(dev, cmd, sizeof(cmd));

    uint8_t cmd2[3] = {0x21, 0, 127};
    ssd1306_write_cmd(dev, cmd2, sizeof(cmd2));
    cmd2[0] = 0x22;
    cmd2[2] = 7;
    ssd1306_write_cmd(dev, cmd2, sizeof(cmd2));

    ret = ssd1306_write_cmd_byte(dev, 0xAF);
...
esp_err_t ssd1306_refresh_gram(ssd1306_handle_t dev) {
    ...
    return ssd1306_write_data(
        dev, &device->s_chDisplayBuffer[0][0], sizeof(device->s_chDisplayBuffer)
    );
}
...
void ssd1306_clear_screen(ssd1306_handle_t dev, uint8_t chFill) {
    ssd1306_dev_t *device = (ssd1306_dev_t *)dev;
    memset(
        device->s_chDisplayBuffer, chFill, sizeof(device->s_chDisplayBuffer)
    );
}

void ssd1306_display_on(ssd1306_handle_t dev) {
    ssd1306_write_cmd_byte(dev, 0xAF);
}
