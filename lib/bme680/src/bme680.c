...
bool bme680_get_results_fixed(bme680_sensor_t *dev, bme680_values_fixed_t *results)
{
    ...
    debug_dev("Fixed point sensor valus - %ld ms: %d/100 C, %ld/1000 Percent, %ld Pascal, %ld Ohm",
...
bool bme680_use_heater_profile(bme680_sensor_t *dev, int8_t profile)
{
    ...
    reg = bme_set_reg_bit(reg, BME680_RUN_GAS, (profile != BME680_HEATER_NOT_USED && dev->settings.heater_temperature[profile] && dev->settings.heater_duration[profile]));
...
bool bme680_set_ambient_temperature(bme680_sensor_t *dev, int16_t ambient)
{
    ...
    {
        data[i] = dev->settings.heater_temperature[i] ? bme680_heater_resistance(dev, dev->settings.heater_temperature[i]) : 0;
    }
...
static bool bme680_get_raw_data(bme680_sensor_t *dev, bme680_raw_data_t *raw_data)
{
    ...
    */
    debug("Raw data: %ld %ld %d %d %d", __FUNCTION__,
