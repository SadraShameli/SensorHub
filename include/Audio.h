...
/**
 * @namespace
 * @brief A namespace for managing microphone related functionality.
 */
namespace Mic {

/** @struct
 * @brief A struct containing the coefficients of a filter.
 */
struct Coefficients {
    float B1 = 0.0f;
    float B2 = 0.0f;
...
/** @struct
 * @brief A struct containing the delay states of a filter.
 */
struct DelayStates {
    float W0 = 0.0f;
    float W1 = 0.0f;
...
/** @class
 * @brief A class for filtering audio data.
 */
class AudioFilter {
   public:
...
/**
 * @brief Normalizes the audio samples to the full range of `int16_t`.
 *
 * This function takes an array of audio samples and normalizes them so that the
 * maximum absolute value of the samples is scaled to the maximum value of
 * `int16_t` (32767).
 *
...
/**
 * @brief Calculates the Root Mean Square (RMS) of an array of values.
 *
 * This function computes the RMS value of the input array. The RMS value is a
 * measure of the magnitude of a set of values, and is commonly used in signal
 * processing.
 *
 * @tparam T The type of the input values.
 * @param input Pointer to the array of input values.
...
//     -0.491647169337140,
//     {
//         {+1.4604385758204708, +0.5275070373815286, +1.9946144559930252,
//         -0.9946217070140883},
