...
static esp_err_t http_resp_dir_html(httpd_req_t* req, const char*) {
...
static esp_err_t config_handler(httpd_req_t* req) {
    static const int kMaxConfigPayload = 4096;

    if (req->content_len <= 0 || req->content_len > kMaxConfigPayload) {
        httpd_resp_send_err(req,
                            HTTPD_400_BAD_REQUEST,
                            "Invalid or oversized configuration payload");
        return ESP_FAIL;
    }
    ...
    bool ok = Backend::SetupConfiguration(http_payload);
    if (!ok) {
        httpd_resp_send_err(req,
                            HTTPD_400_BAD_REQUEST,
    ...
    httpd_resp_sendstr(req, "{\"state\":\"saving\"}");
    return ESP_OK;
}

static esp_err_t status_handler(httpd_req_t* req) {
    Network::ConfigStatusSnapshot snapshot = Network::GetConfigStatus();

    JsonDocument doc;
    doc["state"] = Network::ConfigStateName(snapshot.state);
